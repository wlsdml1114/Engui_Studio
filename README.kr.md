...
EnguiStudio는 자동 마이그레이션 시스템을 포함하고 있습니다:

### 자동 마이그레이션
- **시작 시 자동 실행**: `start-windows.bat` 또는 `start-macos.sh` 실행 시 자동으로 마이그레이션 수행
- **데이터 보호**: 마이그레이션 전 자동 백업 생성
- **안전장치**: 마이그레이션 실패 시 자동으로 백업에서 복원
...
### 마이그레이션 API 엔드포인트
- `GET /api/migration` - 마이그레이션 상태 확인
- `POST /api/migration` - 마이그레이션 실행 (action: "run-migration")
- `POST /api/migration` - 데이터 무결성 검사 (action: "validate-data")

### 데이터 손실 방지
- ✅ **자동 백업**: 마이그레이션 전 타임스탬프가 포함된 백업 파일 생성
- ✅ **롤백 기능**: 마이그레이션 실패 시 자동으로 이전 상태로 복원
- ✅ **데이터 검증**: 마이그레이션 후 데이터 무결성 자동 검사
- ✅ **백업 정리**: 30일 이상 된 백업 파일 자동 정리
...
이 프로젝트는 Engui Studio 라이선스 하에 배포됩니다. 자세한 내용은 [LICENSE](LICENSE) 파일을 참조하세요.
