...
export default function MyPresetsPage() {
  ...
  if (swrError) return <div>Failed to load presets</div>;
  if (!data) return <div>Loading presets...</div>;
  ...
  return (
    <div className="p-4">
      ...
      <div className="mb-6 p-4 border rounded-md bg-gray-700">
        <h2 className="text-xl font-semibold mb-3">Create New Preset</h2>
        <form onSubmit={handleCreatePreset} className="space-y-3">
          <div>
            <label htmlFor="presetName" className="block text-sm font-medium text-gray-300">Preset Name</label>
            <input
              type="text"
              id="presetName"
          ...
          <div>
            <label htmlFor="presetType" className="block text-sm font-medium text-gray-300">Type</label>
            <select
              id="presetType"
              className="mt-1 block w-full rounded-md bg-gray-600 border-gray-500 text-white shadow-sm sm:text-sm"
              value={newPresetType}
          ...
          <div>
            <label htmlFor="presetOptions" className="block text-sm font-medium text-gray-300">Options (JSON)</label>
            <textarea
              id="presetOptions"
              rows={3}
              className="mt-1 block w-full rounded-md bg-gray-600 border-gray-500 text-white shadow-sm sm:text-sm"
      ...
      <h2 className="text-xl font-semibold mb-3">Existing Presets</h2>
      {presets.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {presets.map((preset) => (
            <div key={preset.id} className="border p-4 rounded-md bg-gray-700 shadow-sm">
              ...
              <p className="text-sm text-gray-400">Type: {preset.type}</p>
              <p className="text-sm text-gray-400">Options: {preset.options}</p>
              <button
